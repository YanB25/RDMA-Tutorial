...
void *client_thread_func(void *arg)
{
    ...
    log_info("client: receive start signal.");
    ret = post_recv(msg_size, lkey, (uint64_t) buf_ptr, qp, buf_ptr);
    check(ret == 0, "thread[%ld]: failed to receive start signal", thread_id);
    ...
    while (stop != true)
    {
        ...
        for (i = 0; i < n; i++)
        {
            if (wc[i].status != IBV_WC_SUCCESS)
            {
                ...
                {
                    check(0,
                          "thread[%ld]: send failed status: %s",
                          thread_id,
                          ibv_wc_status_str(wc[i].status));
                }
                else
                {
                    check(0,
                          "thread[%ld]: recv failed status: %s",
                          thread_id,
                          ibv_wc_status_str(wc[i].status));
                }
            }
...
int run_client()
{
    ...
    {
        ret = pthread_create(
            &client_threads[i], &attr, client_thread_func, (void *) i);
